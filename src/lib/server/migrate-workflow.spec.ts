...
describe('.github/workflows/ci.yml', () => {
	...
	it('migrate: fails closed without the secret, installs without lifecycle scripts, migrates then prints status, never seeds', () => {
		const migrate = ci.jobs.migrate;
		...
		expect(
			runs.some((run) => run.includes('pnpm install --frozen-lockfile --ignore-scripts'))
		).toBe(true);
