...
- **Layout 2026-09-14 — the app is the repo root.** `apps/web/` is gone:
  `src/`, `static/`, `messages/`, `project.inlang/`, `drizzle/`, `e2e/`,
  `tests/`, `svelte.config.js`, `vite.config.ts`, `vercel.json` and the app's
  `package.json` (scripts + dependencies, merged into the workspace root)
  now live at the top level; the app's `scripts/` merged into the root
  `scripts/` next to `backup.sh` / `dev-run.sh`. The workspace is the root
  plus `packages/formcomp`. Why: Vercel's SvelteKit preset keys on
  `svelte.config.js` in the project's root directory — with the app under
  `apps/web` (Root Directory `apps/web`, install `cd ../.. && pnpm install`)
  environment-variable updates were not picked up by the build. Vercel
  project settings to change by hand: **Root Directory empty**, **Install
  Command default**; `ci.yml`'s `vercel build` already runs at the root. Lint
  and Tailwind source detection now start at the root, so `.prettierignore`,
  `eslint.config.js` and `layout.css` (`@source not`) exclude `packages/`,
  `.initialData/`, `content/` and `docs/`; the client CSS is byte-identical
  to the pre-move build. `pnpm check` / `pnpm test:unit` still recurse into
  formcomp (`pnpm -r --if-present`). Historical docs (`CHANGELOG`, `phases/`,
  `fixes/`, `next/`, audits) keep their `apps/web/` paths as written.
...
2. **Vercel project settings** (§12): Root Directory EMPTY and Install
   Command default (the app is the repo root since 2026-09-14), automatic
   production deploys OFF, Build Command `pnpm db:status && pnpm build`, Node 24,
...
| `package.json` | auto | Both script sets present (`articles:from-initialdata`, `products:from-initialdata`, `subscriber:export` + upstream's); dependencies are upstream's patched versions + ours `@fontsource-variable/plus-jakarta-sans` (BS-5). |
...
| `messages/ro.json` | union | Hunk: theirs `admin_settings_invoice_vat_standard_rates`; ours `admin_settings_invoice_vat_rate` + `_hint` dropped (the setting is gone with FIX-12). H-10 hint re-added as `admin_product_vat_rate_hint` under the product VAT select — where the question is still open. |
| `messages/en.json` (mod/del) | deleted | Single catalog. Upstream's `src/lib/messages.spec.ts` rewritten for the policy: `ro.json` is the only catalog, `project.inlang` lists exactly `ro`, no blank values. |
| `src/hooks.server.ts` | theirs | `handleRequestId` → `handleSecurityHeaders` → `handleCsrf` → paraglide → settings → route-id admin guard. Deleted `handleHeaders` + `$lib/server/headers.ts` (+ `headers.spec.ts`, `e2e/headers.e2e.ts`): folded into `security-headers.spec.ts` (the nosniff/referrer/frame family, no report-only header) and `security.e2e.ts` (`/admin/login` carries the family; no HSTS on http). The BS-8 client-address resolution is adapter-node's own `ADDRESS_HEADER` (env-matrix, auto-merged). |
...
| `e2e/frontend.e2e.ts` | theirs | Zero hreflang alternates. |
| `e2e/shop.e2e.ts` | union | CSP-violation guard + the local catalogue (36 cards) / stock cases. `global-setup.ts` (auto): truncate list carries `invoice_submissions`, `pending_refunds` + ours; `admin_audit` is append-only and not truncated upstream either. |
...
Removed: `src/lib/server/headers.spec.ts` and `e2e/headers.e2e.ts`
...
- Drizzle: `git diff --stat better-base-2026-09-05 -- drizzle` →
...
- Single site: `grep -rn "SITE_ID=life\|sites/life\|better_life\|funnel-life"
  ...
  (CHANGELOG excepted) → empty; `messages/` holds `ro.json` only;
