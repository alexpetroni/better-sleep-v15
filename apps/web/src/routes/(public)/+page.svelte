...
<LandingNightMap>
	{#snippet segmentExtra(segment)}
		<!-- BS-6: the deck's "name which SKUs support that phase" — real seeded
		     catalogue products, each linking to its shop page. The load filters
		     to currently visible products (M-5); a segment whose SKUs all went
		     inactive renders no label and no chips. -->
		{#if data.nightMapSkus[segment.key].length > 0}
